...
with open(file_name, "r") as file:
    ...
    found_cheats = set()
    def dijkstra():
        min_heap = [(0, start, "")] # (cost, node, cheat_id)
        d = {(i,j): (float("inf"), set()) for i in range(M) for j in range(N)} # shortest path deltas + cheats set!
        d[start] = (0, set())
        # p = {(i,j): None for i in range(M) for j in range(N)} # prev node in shortest path!
        visited = set()

        COST, CHEATS = 0, 1
        while len(min_heap) > 0:
            cost, u, cheat_id = heapq.heappop(min_heap)
            x, y = u

            # You currently have the shortest path from start to u. Now, for
            # every edge u->v, we want to update the shortest path start->v if
            # cost(start->u + u->v) smaller than current start->v cost!
            for dx, dy in DIRECTIONS:
                neigh_x, neigh_y = x + dx, y + dy
                if not inBounds(neigh_x, neigh_y):
                    continue

                if grid[neigh_x][neigh_y] != WALL:
                    new_cost = cost + 1
                    if new_cost < d[(neigh_x, neigh_y)][COST]:
                        d[(neigh_x, neigh_y)] = (new_cost, set([cheat_id]))
                        heapq.heappush(min_heap, (new_cost, (neigh_x, neigh_y), cheat_id))
                    elif new_cost == d[(neigh_x, neigh_y)][COST]:
                        d[(neigh_x, neigh_y)][CHEATS].add(cheat_id)
                        heapq.heappush(min_heap, (new_cost, (neigh_x, neigh_y), cheat_id))
                    
                elif cheat_id == "": # and cheat_id not in found_cheats:
                    ...
                    vis = set()
                    vis.add((i, j))
                    vis.add((neigh_x, neigh_y))
                    while len(queue) > 0:
                        path_len, cheat_x, cheat_y = queue.popleft()
                        new_cheat_id = f"{x, y}-->{cheat_x, cheat_y}"
                        if path_len > 20:
                            continue  # Invalid!
                        if (
                            inBounds(cheat_x, cheat_y)
                            ...
                            #     )
                            # )
                            new_cost = cost + path_len
                            if new_cost < d[(cheat_x, cheat_y)][COST]:
                                d[(cheat_x, cheat_y)] = (new_cost, set([new_cheat_id]))
                                heapq.heappush(min_heap, (new_cost, (cheat_x, cheat_y), new_cheat_id))
                            elif new_cost == d[(cheat_x, cheat_y)][COST]:
                                d[(cheat_x, cheat_y)][CHEATS].add(new_cheat_id)
                                heapq.heappush(min_heap, (new_cost, (cheat_x, cheat_y), new_cheat_id))
        ...
        while len(fringe) > 0:
            ...
            for dx, dy in DIRECTIONS:
                ...
                if (new_x, new_y) not in visited and grid[new_x][new_y] != WALL:
                    ...
                    heapq.heappush(fringe, (cost + 1, new_x, new_y))
    ...
    while len(fringe) > 0:
        ...
        for dx, dy in DIRECTIONS:
            neigh_x, neigh_y = x + dx, y + dy
            if (neigh_x, neigh_y) in visited or not inBounds(neigh_x, neigh_y) or grid[neigh_x][neigh_y] == WALL:
                continue
    ...
    # # print(f"{ucs(start[0], start[1])=}")
    # found_cheats = set()
    # # for key in d:
    # #     i, j = key
    # #     cost, cheats = d[key]
    # #     if len(cheats - found_cheats) > 0:
    # #         # There is at least one cheat here to benefit from. Hence, compute the cost, and if it happens to be
    # #         # <= GOAL_COST, then we append all to our set of found_cheats :)
    # #         full_cost = cost + ucs(i, j)
    # #         # print(f"{cost=}, ucs({i},{j})={ucs(i, j)}, {cheats=}")
    # #         print(f"{full_cost=}")
    # #         if full_cost <= GOAL_COST:
    # #             print(f"{full_cost=}")
    # #             found_cheats = found_cheats.union(cheats)
    # for i in range(M):
    #     for j in range(N):
    #         if grid[i][j] != TRACK:
    #             continue

    #         # Grab shortest path (w/ cheats) from start to (i, j), then compute remaining shortest path cost as well.
    #         cost, cheats = d[(i, j)]
    #         if cost > GOAL_COST:
    #             continue
    #         if len(cheats - found_cheats) == 0:
